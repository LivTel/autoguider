...
/* call CCD_Setup_Dimensions */
	...
	if(!Andor_Setup_Dimensions(Size_X,Size_Y,Bin_X,Bin_Y,Window_Flags,Window))
	{
		CCD_General_Error();
		return 3;
	}
...
/* do command */
	...
	switch(Command)
	{
		case COMMAND_ID_BIAS:
			fprintf(stdout,"Calling Andor_Exposure_Bias.\n");
			retval = Andor_Exposure_Bias(image_buffer,image_buffer_length);
			break;
		case COMMAND_ID_DARK:
			fprintf(stdout,"Calling Andor_Exposure_Expose with open_shutter FALSE.\n");
			retval = Andor_Exposure_Expose(FALSE,start_time,Exposure_Length,
						       image_buffer,image_buffer_length);
			break;
		case COMMAND_ID_EXPOSURE:
			fprintf(stdout,"Calling Andor_Exposure_Expose with open_shutter TRUE.\n");
			retval = Andor_Exposure_Expose(TRUE,start_time,Exposure_Length,
						       image_buffer,image_buffer_length);
			break;
	...
	if(!Test_Save_Fits_Headers(Exposure_Length,Andor_Setup_Get_NCols(),Andor_Setup_Get_NRows(),Fits_Filename))
	{
...
static void Help(void)
{
	...
	fprintf(stdout,"This program calls Andor_Setup_Dimensions to set up the controller dimensions.\n");
